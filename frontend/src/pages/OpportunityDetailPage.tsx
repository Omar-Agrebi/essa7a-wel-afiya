...
import { ArrowLeft, MapPin, ExternalLink, Copy, Check, CheckCircle, Circle, ChevronRight } from 'lucide-react'
import { getOpportunity, getOpportunities } from '@/api/opportunities'
...
// ── Mini-card for similar opportunities strip ─────────────────────────────────
function SimilarCard({ opp }: { opp: Opportunity }) {
  ...
  return (
    <motion.button
      ...
      className="flex-shrink-0 w-52 text-left p-4 rounded-xl border border-observatory-border transition-all duration-200 hover:border-observatory-primary hover:shadow-[0_0_14px_rgba(30,64,175,0.2)]"
      style={{ background: 'rgba(15,23,42,0.9)' }}
    >
      <span className={clsx('px-2 py-0.5 rounded-full text-xs border mb-2 inline-block font-mono', tc.text, tc.bg, tc.border)}>
        {opp.type}
      </span>
      <p className="text-sm font-medium text-observatory-textPrimary line-clamp-2 leading-snug mb-2">{opp.title}</p>
      {opp.location && (
        <p className="text-xs text-observatory-textMuted flex items-center gap-1">
          <MapPin className="w-3 h-3 flex-shrink-0" />
          <span className="truncate">{opp.location}</span>
        </p>
      )}
      {opp.deadline && (
        <p className="text-xs font-mono text-observatory-textMuted mt-1 opacity-60">{opp.deadline}</p>
      )}
    </motion.button>
  )
}
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setOpportunity(data)
        // Fetch cluster siblings, exclude self
        if (data.cluster_id !== null && data.cluster_id !== undefined) {
          const siblings = await getOpportunities({ cluster_id: data.cluster_id }, 0, 8)
          setSimilar(siblings.filter(o => o.id !== data.id).slice(0, 5))
        }
      } catch (e) {
      ...
      <div className="pt-20 pb-16 max-w-6xl mx-auto px-4">
        ...
        <div className="mb-8">
          <div className="flex flex-wrap gap-2 mb-3">
            ...
            {opportunity.cluster_label && (
              <span className="px-3 py-1 rounded-full text-xs border border-observatory-accent/30 bg-observatory-accent/10 text-observatory-accentLight font-mono flex items-center gap-1">
                ◆ {opportunity.cluster_label}
              </span>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* ── SIMILAR OPPORTUNITIES — replaces old cluster card ── */}
            {similar.length > 0 && (
              <div>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-observatory-accent flex-shrink-0" />
                    <span className="font-mono text-xs font-bold text-observatory-textMuted tracking-wider">
                      SIMILAR IN CLUSTER
                    </span>
                    {opportunity.cluster_label && (
                      <span className="text-xs text-observatory-accentLight italic font-normal hidden sm:inline">
                        "{opportunity.cluster_label}"
                      </span>
                    )}
                  </div>
                  <Link
                    to={`/opportunities?cluster_id=${opportunity.cluster_id}`}
                    className="text-xs text-observatory-accent hover:text-observatory-accentLight font-mono flex items-center gap-0.5 transition-colors"
                  >
          ...
          <div className="space-y-4">
            ...
            {/* ML Insights link — replaces old cluster info card */}
            <GlassCard className="p-5 border-observatory-accent/20">
              <h3 className="font-mono text-xs font-bold text-observatory-textMuted tracking-wider mb-2">
                ML INTELLIGENCE
              </h3>
              <p className="text-xs text-observatory-textMuted leading-relaxed mb-3">
                See how this cluster compares to your full skill profile, and where this opportunity sits in your match universe.
              </p>
              <Link to="/insights">
