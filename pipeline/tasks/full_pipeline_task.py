...
# Module-level cache for the last pipeline run report.
# Survives across background task calls within the same process.
_last_pipeline_report: dict | None = None


def get_last_pipeline_report() -> dict:
    """Return the cached last pipeline report, or a 'never_run' sentinel."""
    if _last_pipeline_report:
        return _last_pipeline_report
