...
class OpportunityRepository(BaseRepository[Opportunity]):
    ...
    async def get(self, id: str) -> Opportunity | None:
        """Alias for get_by_id — used by service layer."""
        return await self.get_by_id(id)

    async def list_all(self, filters, skip: int = 0, limit: int = 50):
        """List opportunities with optional filters and pagination."""
        try:
            stmt = select(Opportunity)
            if filters.type:
                stmt = stmt.where(Opportunity.type == filters.type)
            if filters.category:
                stmt = stmt.where(Opportunity.category == filters.category)
            if filters.keyword:
                pattern = f"%{filters.keyword}%"
                stmt = stmt.where(
                    or_(Opportunity.title.ilike(pattern),
                        Opportunity.description.ilike(pattern))
                )
            if filters.cluster_id is not None:
                stmt = stmt.where(Opportunity.cluster_id == filters.cluster_id)
            if filters.expiring_in_days is not None:
                from datetime import timedelta, timezone, datetime
                today = datetime.now(timezone.utc).date()
                target = today + timedelta(days=filters.expiring_in_days)
                stmt = stmt.where(Opportunity.deadline >= today, Opportunity.deadline <= target)
    ...
    async def upsert_by_url(self, obj_in: dict[str, Any]) -> tuple:
        """
